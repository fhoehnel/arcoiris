...
function loadAndShowTrack() {
	...
	var url = getContextRoot() + "/servlet?command=gpxTrack&filePath=" + encodeURIComponent(filePath) + "&trackNumber=" + currentTrack;
...
function loadAndShowMultipleGPXFiles() {
	...
	var url = getContextRoot() + "/servlet?command=gpxTrack&filePath=" + encodeURIComponent(filePath) + "&trackNumber=0";
...
function loadAndShowWayPoints() {
    ...
    var url = getContextRoot() + "/servlet?command=gpxWayPoints&filePath=" + encodeURIComponent(filePath);
...
function drawAltDistProfile(response) {
    ...
    const formattedDist = formatDecimalNumber(Math.ceil(maxTotalDist));
    
    var distanceElem = document.createElement("td");
    distanceElem.setAttribute("class", "gpsChartText");
    distanceElem.style.textAlign = "right";
    distanceElem.innerHTML = "distance: " + formattedDist + " " + (formattedDist.indexOf(".") >= 0 ? "km" : "m");
...
function drawSpeedProfile(response, speedProperty, averageSpeedInMotionProp) {
    ...
    const averageSpeedElem = document.createElement("td");
    if (response[averageSpeedInMotionProp]) {
    	averageSpeedElem.setAttribute("class", "gpsChartText");
    	averageSpeedElem.innerHTML = "average speed in motion: " + (response[averageSpeedInMotionProp]  * 3.6).toFixed(2) + " km/h";
    }
    rowElem.appendChild(averageSpeedElem);
...
function showTrackInSlowMotion(trackId) {
    ...
    var url = getContextRoot() + "/servlet?command=gpxTrack&filePath=" + encodeURIComponent(filePath) + "&trackNumber=" + trackId;
