...
public class BlogEditEntryHandler extends XslRequestHandlerBase {
    ...
    public BlogEditEntryHandler(HttpServletRequest req, HttpServletResponse resp, HttpSession session, PrintWriter output, String uid) {
    ...
    protected void process() {
        ...
        String srcFileName = req.getContextPath() + "/servlet?command=getFile&filePath=" + UTF8URLEncoder.encode(imgPath) + "&cached=true";
        XmlUtil.setChildText(blogEntryElement, "imgPath", srcFileName);

        if (origImgPath != null) {
            String origImgSrc = req.getContextPath() + "/servlet?command=getFile&filePath=" + UTF8URLEncoder.encode(origImgPath) + "&cached=true";
