...
public class Email extends Thread {
    ...
    private void debug(String message, boolean response) {
        if (ArcoirisBlog.getInstance().isDebugMail()) {
            if (response) {
    ...
    public boolean sendInternal() {
        ...
        for (int i = 0; i < receiverList.size(); i++) {
            ...
            try {
                s = new Socket(ArcoirisBlog.getInstance().getMailHost(), 25);
                ...
                temp = "MAIL FROM: <" + ArcoirisBlog.getInstance().getMailSenderAddress() + "> BODY=7BIT";
                mailOut.print(temp);
                mailOut.print("\r\n");
                mailOut.flush();
                debug(temp, false);
                ...
                if (mailSenderName == null) {
                    mailSenderName = ArcoirisBlog.getInstance().getMailSenderName();
                }

                if (mailSenderAddress == null) {
                    mailSenderAddress = ArcoirisBlog.getInstance().getMailSenderAddress();
                }
    ...
    public String encodeMessageBody() throws Exception {
        ...
        if (ArcoirisBlog.getInstance().getClientUrl() != null) {
            messageBody.append(ArcoirisBlog.getInstance().getClientUrl());
            messageBody.append("\r\n\r\n");
        } else {
            /*
             * if (WebFileSys.getInstance().getHttpServerPort() > 0) {
             * messageBody.append("http://");
             * messageBody.append(InetAddress.getLocalHost().getHostAddress() +
             * ":" + WebFileSys.getInstance().getHttpServerPort() +
             * "/webfilesys/servlet \r\n"); }
             * 
             * if (WebFileSys.getInstance().getSslServerPort() > 0) {
             * messageBody.append("https://");
             * messageBody.append(InetAddress.getLocalHost().getHostAddress() +
             * ":" + WebFileSys.getInstance().getSslServerPort() +
             * "/webfilesys/servlet \r\n"); }
             */
        }

        messageBody.append("---------------------\r\n");
        messageBody.append("powered by arcoiris blog\r\n");
    ...
    public synchronized void getSMTPResponse(DataInputStream in, String expectedResponse) throws IOException {
    ...
    /**
     * MIME encoded-word syntax (RFC 2047). "=?charset?encoding?encoded text?="
     * 
     * @param source
     *            source text
     * @return encoded word
