...
/**
 * This UserManager implementation stores the webfilesys user data in a cached
 * XML file but authenticates users against an LDAP server. If the LDAP
 * authentication succeeds, but the user is unknown to the webfilesys user
 * database, the user is created and some essential user data are copied from
 * LDAP to the webfilesys user db. Some other user attributes will get default
 * values, for example role is always set to webspace.
 * 
 * For the LDAP authentication a LDAP user object with uid matching the userid
 * from the login form is searched in the LDAP tree location specified by
 * property usersBaseDN. If that user object is not found, it is searched in the
 * LDAP organization tree below usersBaseDN. For this search an LDAP Connection
 * with the credentials configured as ldapBindUser and ldapBindPassword is used.
 * If the user object can be found in the LDAP tree, the authentication with the
 * password entered in the login form is done against this LDAP user.
 * 
 * It is possible to configure that LDAP users are required to belong to a
 * certain LDAP group to get access to webfilesys (property
 * webfilesysUserGroup). For checking the group membership the LDAP connection
 * is made with credentials configured as ldapBindUser and ldapBindPassword.
 * 
 * A custom SSL Socket factory can be configured for the LDAP connection. This
 * is useful in test environments where LDAP servers run with self-signed
 * certificates. The TrustAllCertificatesSocketFactory disables the certificate
 * checking.
 * 
...
public class LdapAuthenticatedXmlUserManager extends XmlUserManager {
    ...
    private static final String[] USER_ATTRIB_SET_KEYS = { LDAP_ATTR_FIRST_NAME, LDAP_ATTR_LAST_NAME, LDAP_ATTR_MAIL, LDAP_ATTR_LANGUAGE, LDAP_ATTR_PHONE };
    ...
    protected void initLdap() {
        File ldapConfigFile = new File(ArcoirisBlog.getInstance().getConfigBaseDir(), LDAP_CONFIG_FILE);
    ...
    private boolean createUserFromLdap(DirContext ctx, String searchPath, String userId, String password) {
        ...
        try {
            ...
            if (answer.hasMoreElements()) {
                ...
                user.setDiskQuota(ArcoirisBlog.getInstance().getDefaultDiskQuota());
                // user.setPageSize(WebFileSys.getInstance().getThumbnailsPerPage());
                user.setLastLogin(new Date());

                if (CommonUtils.isEmpty(ArcoirisBlog.getInstance().getUserDocRoot())) {
                    LOG.error("no UserDocumentRoot configured but required to copy LDAP users");
                    return false;
                }

                String homeDir = ArcoirisBlog.getInstance().getUserDocRoot() + File.separator + userId;
